...
#
# listusers = api.do_get('/xhr/user', {'sort[limit]': '10', 'sort[order]': 'username'})
#
...
#
# adduser = api.do_post('/xhr/user', {'username' : 'foobar', 'role_id': 4, 'domain': 'mydomain', 'password': ''})
#
...
class MSUIAPIClient:
    ...
    def do_post(self, rel_url, post_data, params=None):
        ...
        result = self.session.post(full_url, json=post_data)
        if result.status_code == 200:
            return result.json()
        else:
            raise Exception
